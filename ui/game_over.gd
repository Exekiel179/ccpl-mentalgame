...
func _show_stats() -> void:
	...
	var mode: int = int(stats.get("mode", GameManager.current_mode))
	if mode == GameManager.GameMode.CARD:
		_add_stat(container, "完成危机卡: %d / %d" % [int(stats.get("completed_encounters", 0)), int(stats.get("total_encounters", 0))], Color(0.80, 0.92, 1.0), 18)
		_add_stat(container, "心理值: %d   情绪温度: %d" % [int(stats.get("health", 0)), int(stats.get("stress", 0))], Color(0.72, 0.72, 0.82), 15)
		var emotion_label := String(stats.get("emotion_label", ""))
		if not emotion_label.is_empty():
			_add_stat(container, "%s：%d → %d" % [emotion_label, int(stats.get("emotion_before", 0)), int(stats.get("emotion_after", 0))], Color(1.0, 0.82, 0.52), 15)
		return
	...
	_add_stat(container, "被碰到: %d  ·  成功躲避: %d" % [stats.get("touched", 0), stats.get("dodged", 0)], Color(0.72, 0.72, 0.82), 14)
	...
	if wrong_list.size() > 0:
		...
		for item in wrong_list:
			...
			_add_stat(container, "• " + d.get("text", "") + " — " + d.get("explanation", ""), Color(0.78, 0.78, 0.86), 13)
